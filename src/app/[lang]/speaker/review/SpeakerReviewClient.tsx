...
export default function SpeakerReviewClient({ locale }: Props) {
  ...
  const previewTalk = useMemo<Talk>(() => ({
    ...
    title: draft.titleJa || draft.titleEn || "タイトルプレビュー",
    speaker: draft.speakerJa || draft.speakerEn || "スピーカー名",
    abstract: draft.abstractJa || draft.abstractEn,
    speakerBio: draft.bioJa || draft.bioEn,
  ...
  const snippet = useMemo(() => {
    ...
    return [
      ...
      `  title: ${codeString(draft.titleJa || draft.titleEn || "TBD")},`,
      `  speaker: ${codeString(draft.speakerJa || draft.speakerEn || "TBD")},`,
      `  abstract: ${codeString(replacedDraft.abstractJa || replacedDraft.abstractEn || "詳細は後日公開予定です。")},`,
      `  speakerBio: ${codeString(replacedDraft.bioJa || replacedDraft.bioEn || "")},`,
      replacedDraft.speakerImage.trim() ? `  speakerImage: ${codeString(replacedDraft.speakerImage)},` : "",
      `  report: ${codeString("レポートは後日公開予定です。")},`,
